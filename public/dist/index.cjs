 ...
 */
class VRMRenderer extends EventEmitter {
    constructor(avatarPath, canvas) {
        ...
        this.camera = new THREE__namespace.PerspectiveCamera(50, 1, 0.05, 50.0);
        ...
        this.resize();
        // ライティング
        const directionalLight = new THREE__namespace.DirectionalLight(0xffffff, 1.8);
        directionalLight.position.set(1.2, 2.5, 1.8);
        this.scene.add(directionalLight);
        const fillLight = new THREE__namespace.DirectionalLight(0xffffff, 0.6);
        fillLight.position.set(-1, 1.2, 1.2);
        this.scene.add(fillLight);
        const ambientLight = new THREE__namespace.HemisphereLight(0xffffff, 0x8899aa, 1.0);
    ...
    async loadVRM(path) {
        try {
            ...
            // 最適化
            threeVrm.VRMUtils.removeUnnecessaryVertices(vrm.scene);
            threeVrm.VRMUtils.removeUnnecessaryJoints(vrm.scene);
            ...
            this.vrm = vrm;
            // アニメーションミキサーの初期化
            this.mixer = new THREE__namespace.AnimationMixer(vrm.scene);
...
class LipSync {
    constructor(renderer) {
        ...
        this.update = () => {
            ...
            if (vrm?.expressionManager) {
                ...
                vrm.expressionManager.setValue(threeVrm.VRMExpressionPresetName.Aa, weight);
            }
    ...
    stop() {
        ...
        const vrm = this.renderer.getVRM();
        if (vrm?.expressionManager) {
            vrm.expressionManager.setValue(threeVrm.VRMExpressionPresetName.Aa, 0);
        }
    }
}
...
class ExpressionManager {
    ...
    setJoy(weight) {
        this.setExpression(threeVrm.VRMExpressionPresetName.Happy, weight);
    }
    setAngry(weight) {
        this.setExpression(threeVrm.VRMExpressionPresetName.Angry, weight);
    }
    setSorrow(weight) {
        this.setExpression(threeVrm.VRMExpressionPresetName.Sad, weight);
    }
    setFun(weight) {
        this.setExpression(threeVrm.VRMExpressionPresetName.Happy, weight * 0.5);
    }
    setNeutral() {
        const vrm = this.renderer.getVRM();
        if (vrm?.expressionManager) {
            ...
            const presets = [
                threeVrm.VRMExpressionPresetName.Happy,
                threeVrm.VRMExpressionPresetName.Angry,
                threeVrm.VRMExpressionPresetName.Sad,
            ];
...
class AnimationManager {
    ...
    async playAnimation(path) {
        ...
        try {
            ...
            action.reset();
            action.setLoop(THREE__namespace.LoopOnce, 0);
                ...
                const handleFinished = (e) => {
                    if (e?.action === action) {
                        ...
                        if (this.idleAction && this.idleAction !== action) {
                            ...
                            this.idleAction.setLoop(THREE__namespace.LoopRepeat, Infinity);
    ...
    async getOrLoadClip(path) {
        ...
        try {
            ...
            if (!vrm)
                return null;
            const clip = threeVrmAnimation.createVRMAnimationClip(vrmAnimation, vrm);
...
class BlinkController {
    constructor(renderer) {
        ...
        this.update = () => {
            ...
            const currentHappyWeight = vrm.expressionManager.getValue(threeVrm.VRMExpressionPresetName.Happy) || 0;
            const currentAngryWeight = vrm.expressionManager.getValue(threeVrm.VRMExpressionPresetName.Angry) || 0;
            const currentSadWeight = vrm.expressionManager.getValue(threeVrm.VRMExpressionPresetName.Sad) || 0;
    ...
    blink() {
        ...
        vrm.expressionManager.setValue(threeVrm.VRMExpressionPresetName.Blink, blinkWeight);
        setTimeout(() => {
            if (vrm?.expressionManager) {
                vrm.expressionManager.setValue(threeVrm.VRMExpressionPresetName.Blink, 0);
            }
        }, blinkDuration);
    }
}
 ...
 * ```
 */
function getAnimationPath(animationName) {
 ...
 * ```
 */
function getAvatarPath(avatarName) {
    return `/assets/avatars/${avatarName}`;
}

exports.AvatarSpeaker = AvatarSpeaker;
exports.getAnimationPath = getAnimationPath;
